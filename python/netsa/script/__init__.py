    ...
    **Example:** Separately process each hour's SMTP data from the
    ...
    def _check_params(self):
        ...
        if self._input_pipe != None:
            input_sources += 1
            if not isinstance(self._input_pipe, str):
                script_error = ("input_pipe argument %s is not a str" %
                                repr(self._input_pipe))
                raise script_error
        if self._xargs != None:
            input_sources += 1
            if not isinstance(self._xargs, str):
                script_error = ("xargs argument %s is not a str" %
                                repr(self._xargs))
                raise script_error
            ...
            try:
                for x in self._filenames:
                    if not isinstance(x, str):
                        script_error = ("filenames argument %s is not a list "
                                        "of str" % repr(self._filenames))
                        raise script_error
            except TypeError:
                script_error = ("filenames argument %s is not a list "
                                "of str" % repr(self._filenames))
        ...
        if (self._class != None or
            ...
            input_sources += 1
            if self._class != None:
                if not isinstance(self._class, str):
                    script_error = ("flow_class argument %s is not a str" %
                                    repr(self._class))
                    raise script_error
                    if ',' in self._class:
                        script_error = ("flow_class argument %s contains ','" %
                                        repr(self._class))
                        raise script_error
            if self._type != None:
                try:
                    for x in self._type:
                        if not isinstance(x, str):
                            script_error = ("type argument %s is not a "
                                            "list of str"
                                            % repr(self._type))
                            raise script_error
                except TypeError:
                    script_error = ("flowtypes argument %s is not a list of "
                                    "str/str" % repr(self._class))
                    raise script_error
            if self._flowtypes != None:
                try:
                    for x in self._flowtypes:
                        if not isinstance(x, str) or '/' not in x:
                            script_error = ("flowtypes argument %s is not a "
                                            "list of str/str"
                                            % repr(self._class))
                            raise script_error
                except TypeError:
            ...
            if self._sensors != None:
                try:
                    for x in self._sensors:
                        if not isinstance(x, str):
                            script_error = ("sensors argument %s is not a list "
                                            "of str" % repr(self._sensors))
                            raise script_error
                except TypeError:
                    script_error = ("sensors argument %s is not a list "
                                    "of str" % repr(self._sensors))
            ...
            if self._start_date != None:
                if not isinstance(self._start_date, datetime.datetime):
                    script_error = ("start_date argument %s is not a "
                                    "datetime" % repr(self._start_date))
                    raise script_error
            if self._end_date != None:
                if not isinstance(self._end_date, datetime.datetime):
                    script_error = ("end_date argument %s is not a datetime" %
                                    repr(self._end_date))
                    raise script_error
            if self._start_date != None and self._end_date != None:
                if self._end_date < self._start_date:
                    script_error = ("end_date %s precedes start_date %s" %
