...
def dow_day_snapper(size, dow=0):
    """
    Given an integer size in days and an integer day-of-the-week,
    returns a :class:``DateSnapper`` object anchored on the first
    occurring instance of that DOW after the UNIX epoch. Monday is the
    0th DOW.  DOW values are modulo 7, so the 7th DOW would also
    represent Monday.
    """
    offset = (dow + (7 - (DT_EPOCH.weekday()))) % 7
    epoch  = DT_EPOCH + timedelta(days=offset)
    return DateSnapper(timedelta(days=size), epoch=epoch)
